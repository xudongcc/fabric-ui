...
export const Preview = async ({
  ...
  const parsedCode = code
    ...
    .replace(/@repo\//g, "@/components/fabric-ui/")

    // Remove typography import
    .replace(
      /^import\s+["']@\/components\/ui\/fabric-ui\/typography["'];?\n?/gm,
      "",
    );

  const sourceComponentNames =
    parsedCode
      .match(/@\/components\/ui\/fabric-ui\/([^'"`]+)/g)
      ?.map((match) => match.replace("@/components/fabric-ui/", "")) || [];
